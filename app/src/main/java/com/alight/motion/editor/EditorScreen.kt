...
@Composable fun EditorScreen(project: Project) {
    ...
    LaunchedEffect(playing) {
        if (playing) while (playing && time < project.duration) { delay(16L); time = (time + 0.016f).coerceAtMost(project.duration) }
        if (time >= project.duration) { playing = false; time = 0f }
    }
    ...
    Column(Modifier.fillMaxSize().background(Color(0xFF151518)).systemBarsPadding()) {
        Surface(color = Color(0xFF202025), shadowElevation = 4.dp) {
            Row(Modifier.fillMaxWidth().height(44.dp).padding(horizontal = 6.dp), verticalAlignment = Alignment.CenterVertically) {
                IconButton({}, Modifier.size(32.dp)) { Icon(Icons.Filled.Menu, null, tint = Color(0xFFAAAAAA), modifier = Modifier.size(18.dp)) }
                Column(Modifier.padding(horizontal = 4.dp).weight(1f)) {
                    Text(project.name, color = Color(0xFFE0E0E0), fontSize = 13.sp, fontWeight = FontWeight.SemiBold)
                    Text("${project.width}x${project.height} ${project.fps}fps", color = Color(0xFF666670), fontSize = 10.sp, fontFamily = FontFamily.Monospace)
                }
                Tool.entries.forEach { t ->
                    IconButton({ tool = t }, Modifier.size(30.dp)) {
                        Icon(when (t) { Tool.SELECT -> Icons.Filled.NearMe; Tool.PEN -> Icons.Filled.Draw; Tool.SHAPE -> Icons.Filled.AutoAwesome; Tool.TEXT -> Icons.Filled.TextFields; Tool.HAND -> Icons.Filled.PanTool }, t.name, tint = if (tool == t) Color(0xFF5C9CFF) else Color(0xFF6A6A72), modifier = Modifier.size(15.dp))
                    }
                }
                VerticalDivider(Modifier.height(20.dp).padding(horizontal = 4.dp), color = Color(0xFF2A2A30))
                EditorPanel.entries.forEach { p ->
                    TextButton({ panel = p }, Modifier.height(28.dp), contentPadding = PaddingValues(horizontal = 6.dp)) {
                        Text(p.label, fontSize = 9.sp, fontFamily = FontFamily.Monospace, letterSpacing = 1.sp, color = if (panel == p) Color.White else Color(0xFF555560))
                    }
                }
        ...
        Row(Modifier.weight(1f).fillMaxWidth()) {
            Surface(color = Color(0xFF18181D), modifier = Modifier.width(220.dp)) {
                Column(Modifier.fillMaxSize()) {
                    ...
                    Divider(color = Color(0xFF25252A))
                    LazyColumn {
                            ...
                            item {
                                Column(Modifier.fillMaxWidth()) {
                                    ...
                                    track.layers.forEach { l ->
                                        Row(Modifier.fillMaxWidth().padding(start = 20.dp, end = 4.dp).clip(RoundedCornerShape(3.dp)).background(if (l.id == selId) Color(0xFF2A2A44) else Color.Transparent).clickable { selId = l.id }.padding(horizontal = 6.dp, vertical = 4.dp), verticalAlignment = Alignment.CenterVertically) {
                                            ...
                                            Text(l.name, color = if (l.id == selId) Color.White else Color(0xFF999999), fontSize = 10.sp, maxLines = 1, modifier = Modifier.weight(1f))
                                            Text("%.1fs".format(l.duration), color = Color(0xFF4A4A50), fontSize = 7.sp, fontFamily = FontFamily.Monospace)
                                        }
                                    }
                                }
                            }
            ...
            Box(Modifier.weight(1f).fillMaxHeight().background(Color(0xFF0A0A0D)), contentAlignment = Alignment.Center) {
                ...
                Column(Modifier.horizontalScroll(rememberScrollState())) {
                    project.tracks.filter { it.isVisible }.forEach { track ->
                        track.layers.filter { it.isVisible && time in it.startTime..it.startTime + it.duration }.forEach { l ->
                            val tf = KeyframeEngine.evalTransform(l.keyframes, time - l.startTime, l.transform)
                            val op = KeyframeEngine.evalOpacity(l.keyframes, time - l.startTime, l.opacity)
            ...
            AnimatedContent(panel, Modifier.width(260.dp)) { p ->
                Surface(color = Color(0xFF18181D)) {
                    Column(Modifier.fillMaxSize().padding(10.dp)) {
                        when (p) {
                            ...
                            EditorPanel.EXPORT -> ExportConfigPanel(project) {}
                        }
                    }
                }
            }
