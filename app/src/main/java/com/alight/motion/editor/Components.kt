...
@Composable fun ExportConfigPanel(project: Project, onExport: () -> Unit) {
    ...
    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(4.dp)) { ExportFormat.entries.forEach { f -> Surface(color=if(fmt==f)Color(0xFF3A3A50)else Color.Transparent,shape=RoundedCornerShape(12.dp),modifier=Modifier.clickable{fmt=f}.padding(horizontal=8.dp,vertical=4.dp)){Text(f.name,fontSize=10.sp,fontFamily=FontFamily.Monospace,color=if(fmt==f)Color.White else Color(0xFF666670))} } }
    ...
    Row(Modifier.fillMaxWidth()) { ALL_FPS.take(7).forEach { f -> Surface(color=if(fps==f)Color(0xFF3A3A50)else Color.Transparent,shape=RoundedCornerShape(12.dp),modifier=Modifier.clickable{fps=f}.padding(horizontal=6.dp,vertical=2.dp)){Text("$f",fontSize=9.sp,fontFamily=FontFamily.Monospace,color=if(fps==f)Color.White else Color(0xFF666670))} } }
...
@Composable fun ResolutionDialog(project: Project, onDismiss: () -> Unit) {
    AlertDialog(onDismissRequest = onDismiss, title = { Text("Project Settings", color = Color.White) }, text = {
        Column {
            ...
            Row(Modifier.fillMaxWidth()) { ALL_FPS.forEach { f -> Surface(color=if(project.fps==f)Color(0xFF3A3A50)else Color.Transparent,shape=RoundedCornerShape(12.dp),modifier=Modifier.clickable{project.fps=f}.padding(horizontal=6.dp,vertical=2.dp)){Text("$f",fontSize=9.sp,fontFamily=FontFamily.Monospace,color=if(project.fps==f)Color.White else Color(0xFF666670))} } }
        }
